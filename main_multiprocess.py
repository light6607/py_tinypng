...
import os
import typing
from utils.func_time import clock, times_used
...
def main():
    ...
    for filename in file_list:
        args_item = (f'{in_path}/{filename}', f'{out_path}/{filename}')
        # compress_png(in_file=f'{in_path}/{filename}', out_file=f'{out_path}/{filename}')
        args_list.append(args_item)

    with Pool(cpu_worker) as process_poor:
        process_poor.starmap(compress_png, args_list)
